...
export async function generateMetadata({
  ...
  return {
    ...
    title: t("caseStudyC.metadata.title"),
    description: t("caseStudyC.metadata.description"),
    openGraph: {
      title: t("caseStudyC.metadata.title"),
      description: t("caseStudyC.metadata.description"),
...
export default async function BedrijfCPage() {
  const t = await getTranslations("resultaten");

  const maand1Items = t.raw("caseStudyC.maand1Items") as string[];
  const maand2Items = t.raw("caseStudyC.maand2Items") as string[];
  const maand5Items = t.raw("caseStudyC.maand5Items") as string[];
  const maand10Items = t.raw("caseStudyC.maand10Items") as string[];
  const tableRows = t.raw("caseStudyC.tableRows") as Array<{
    metric: string;
    voor: string;
    na: string;
    verschil: string;
            ...
            <div className="max-w-4xl">
              ...
              <div className="bg-white rounded-2xl p-8 md:p-12 shadow-sm">
                ...
                <div className="space-y-8">
                  ...
                  <div>
                    <h3 className="font-display text-2xl text-text-dark mb-4">
                      {t("caseStudyC.maand10Title")}
                    </h3>
                    <ul className="space-y-3 text-text-dark-secondary">
                      {maand10Items.map((item, i) => (
                        <li key={i} className="flex items-start gap-3">
                          <span className="text-accent-teal-dark font-bold flex-shrink-0">
                            &rarr;
                          </span>
                ...
                <table className="w-full">
                  ...
                  <tbody className="text-text-dark-secondary">
                    {tableRows.map((row, i) => (
                      ...
                      >
                        <td
                          className={`py-4 pr-4 ${i === 0 ? "font-display text-lg" : ""}`}
                        >
                          {row.metric}
                        </td>
                        <td
                          className={`py-4 px-4 ${i === 0 ? "font-display text-lg" : ""}`}
                        >
                          {row.voor}
                        </td>
                        <td
                        ...
                        <td
                          className={`py-4 pl-4 font-bold text-accent-teal-dark ${i === 0 ? "font-display text-lg" : ""}`}
                        >
                          {row.verschil}
                        </td>
                      </tr>
            ...
            <div className="max-w-4xl">
              <div className="bg-accent-teal-dark/5 border-l-4 border-accent-teal-dark rounded-r-2xl p-8 md:p-12">
                <blockquote className="text-xl md:text-2xl text-text-dark italic leading-relaxed mb-6">
                  &ldquo;{t("caseStudyC.quoteDetailed")}&rdquo;
                </blockquote>
                <div className="font-medium text-text-dark">
                  {t("caseStudyC.quoteRole")}
                </div>
              </div>
            </div>
          </FadeIn>
