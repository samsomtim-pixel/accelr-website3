...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "blogTools.metadata" });

  return {
    alternates: buildAlternates("/blog/tools-signal-based-outbound", locale),
    title: t("title"),
    description: t("description"),
    openGraph: {
      title: t("title"),
      description: t("description"),
      url: pageUrl("/blog/tools-signal-based-outbound", locale),
...
export default async function BlogPost({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <JsonLd data={[blogPostingSchema({ slug: "tools-signal-based-outbound", headline: t("metadata.title"), description: t("metadata.description"), locale: __locale }), breadcrumbSchema([{ name: "Home", path: "/" }, { name: "Blog", path: "/blog" }, { name: t("metadata.title"), path: "/blog/tools-signal-based-outbound" }], __locale)]} />
      <Navbar />
      <main className="pt-[72px]">
        ...
        <RelatedServices
          ...
          services={[{ href: "/outbound-leadgeneratie", title: __locale === "en" ? "Outbound lead generation" : "Outbound leadgeneratie", desc: __locale === "en" ? "Signal-based outbound that books meetings." : "Signal-based outbound dat afspraken oplevert." }, { href: "/ai-automation", title: __locale === "en" ? "AI & Automation" : "AI & Automation", desc: __locale === "en" ? "Embed AI in your daily sales process." : "AI inbedden in je dagelijkse salesproces." }]}
        />
