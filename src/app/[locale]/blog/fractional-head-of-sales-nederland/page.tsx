...
export default async function BlogPost({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <>
      <JsonLd data={[blogPostingSchema({ slug: "fractional-head-of-sales-nederland", headline: t("metadata.title"), description: t("metadata.description"), locale: __locale }), breadcrumbSchema([{ name: "Home", path: "/" }, { name: "Blog", path: "/blog" }, { name: t("metadata.title"), path: "/blog/fractional-head-of-sales-nederland" }], __locale)]} />
      <Navbar />
