...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "salesStrategie" });

  return {
    alternates: buildAlternates("/sales-strategie", locale),
    title: t("metadata.title"),
    description: t("metadata.description"),
    openGraph: {
      title: t("metadata.title"),
      description: t("metadata.description"),
      url: pageUrl("/sales-strategie", locale),
...
export default async function SalesStrategiePage() {
  ...
  return (
    <>
      <JsonLd data={[serviceSchema({ name: t("metadata.title"), description: t("metadata.description"), path: "/sales-strategie" }), faqPageSchema(((): Array<{q:string;a:string}> => { const raw = t.raw("faq") as { items?: Array<{q:string;a:string}> } | Array<{q:string;a:string}>; return Array.isArray(raw) ? raw : (raw.items ?? []); })())]} />
      <Navbar />
