...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "outbound" });

  return {
    alternates: buildAlternates("/outbound-leadgeneratie", locale),
    title: t("metadata.title"),
    description: t("metadata.description"),
    openGraph: {
      title: t("metadata.title"),
      description: t("metadata.description"),
      url: pageUrl("/outbound-leadgeneratie", locale),
...
const tools = ["Clay", "Apollo", "LinkedIn Sales Nav", "Instantly", "Lemlist", "Leadinfo", "HubSpot"];

export default async function OutboundLeadgeneratiePage() {
  ...
  return (
    <>
      <JsonLd data={[serviceSchema({ name: t("metadata.title"), description: t("metadata.description"), path: "/outbound-leadgeneratie" }), faqPageSchema(((): Array<{q:string;a:string}> => { const raw = t.raw("faq") as { items?: Array<{q:string;a:string}> } | Array<{q:string;a:string}>; return Array.isArray(raw) ? raw : (raw.items ?? []); })())]} />
      <Navbar />
