...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "crmImplementatie" });

  return {
    alternates: buildAlternates("/crm-implementatie", locale),
    title: t("metadata.title"),
    description: t("metadata.description"),
    openGraph: {
      title: t("metadata.title"),
      description: t("metadata.description"),
      url: pageUrl("/crm-implementatie", locale),
...
export default async function CrmImplementatiePage() {
  ...
  return (
    <>
      <JsonLd data={[serviceSchema({ name: t("metadata.title"), description: t("metadata.description"), path: "/crm-implementatie" }), faqPageSchema(((): Array<{q:string;a:string}> => { const raw = t.raw("faq") as { items?: Array<{q:string;a:string}> } | Array<{q:string;a:string}>; return Array.isArray(raw) ? raw : (raw.items ?? []); })())]} />
      <Navbar />
